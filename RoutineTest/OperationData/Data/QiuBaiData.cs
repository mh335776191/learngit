...
namespace OperationData.Data
{
    public class QiuBaiData : BaseData
    {
        ...
        private readonly Regex DetailReg = new Regex("(<div[^>]*?class=\"author[^>]*>[\\s\\S]*?<img[^>]*alt=\"(?<author>[^\"]*)\"[\\s\\S]*?</div>\\s*)?(?<content><div[^>]*?class=\"content\"\\s*title=\"(?<date>[^>]*)\"[^>]*>[\\s\\S]*?</div>\\s*(<div[^>]*class=\"thumb\">[\\s\\S]*?</div>)?)\\s*<div[^>]*?class=\"stats clearfix\"[^>]*?>[\\s\\S]*?<i[^>]*class=\"number\"[^>]*>(?<goodnum>\\d+)</i>[\\s\\S]*?<span[^>]*?class=\"stats-comments\"[^>]*>[\\s\\S]*?<i[^>]*?class=\"number\"[^>]*>(?<replynum>\\d+)</i>", RegexOptions.IgnoreCase);
